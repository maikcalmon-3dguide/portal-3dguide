"""
3D Guide Planning — Portal Premium do Cliente (v14_client.py) V7-Supabase
Landing Page com transição para Formulário.
Backend: Supabase (PostgreSQL + Storage).

Instalação:  pip install streamlit supabase
Executar:    streamlit run v14_client.py
...
Arquivos na mesma pasta:
    logo_planning.png   — logo 3D Guide (versão light vibrante)
    fundo.jpeg          — imagem tecnológica do dente digital
"""
...
# ══════════════════════════════════════════════════════════════
# PÁGINA — layout wide para o split-screen da landing
# ══════════════════════════════════════════════════════════════
st.set_page_config(
...
# ══════════════════════════════════════════════════════════════
# SUPABASE — conexão única via session_state
# ══════════════════════════════════════════════════════════════
def get_supa() -> Client:
...
def inserir_pedido(payload: dict) -> str:
    """Insere na tabela pedidos_producao e devolve o id gerado."""
...
def upload_arquivo(file_bytes: bytes, nome_original: str,
                   mime: str = "application/octet-stream") -> str:
    """
    Faz upload para o bucket arquivos_pacientes.
    Retorna a URL pública do arquivo.
    Arquivo salvo com nome único (UUID) para evitar colisões.
    """
    ...
    supa = get_supa()
    supa.storage.from_(_BUCKET).upload(
        caminho,
        file_bytes,
        {"content-type": mime, "upsert": "true"},
    )
    url = supa.storage.from_(_BUCKET).get_public_url(caminho)
    return url
...
_PASSOS = ["Identificação", "Técnico", "Odontograma", "Cirurgia", "Revisar"]

# ══════════════════════════════════════════════════════════════
# CSS — dois modos: landing (wide) e formulário (centered)
# ══════════════════════════════════════════════════════════════
def inject_css(estado: str):
    max_w = "1200px" if estado == "landing" else "740px"
    st.markdown(f"""
    <style>
    ...
    .hero-wrapper {{
        ...
        overflow: hidden;
        box-shadow: 0 6px 32px rgba(14,77,102,.12);
    ...
    .features {{
        display: flex; gap: .5rem; flex-wrap: wrap; margin-top: 1.4rem;
    }}
    .feat-badge {{
    ...
    .prog-line {{ width:52px; height:2px; background:#e5e7eb; margin-bottom:1.5rem; flex-shrink:0; }}
    ...
    .passo-titulo {{
        ...
        padding-bottom: .7rem; border-bottom: 2px solid {_AZUL};
...
def _reset_form():
    for k in [k for k in st.session_state if k.startswith("pp_")]:
...
def _prog(passo):
    ...
    for i, lbl in enumerate(_PASSOS, start=1):
        cls_c = "done" if i < passo else ("active" if i == passo else "")
        ico   = "✓" if i < passo else str(i)
        partes.append(
            f'<div class="prog-step">'
            f'<div class="prog-circle {cls_c}">{ico}</div>'
            f'<div class="prog-label {cls_c}">{lbl}</div>'
            f'</div>')
        if i < total:
            partes.append(
                f'<div class="prog-line {"done" if i < passo else ""}"></div>')
    st.markdown(f'<div class="prog-bar">{"".join(partes)}</div>',
                unsafe_allow_html=True)
...
def render_landing():
    ...
    <div class="hero-wrapper">
        <div class="hero-left">
            ...
            <p>Sua cirurgia começa com a máxima precisão no Portal 3D Guide.
            ...
            preencha as informações com atenção aos detalhes.
            <em>Cada campo é fundamental para o sucesso do seu caso.</em></p>
            <div class="features">
...
def render_formulario():
    ...
    # ── Passo 4 ───────────────────────────────────────────────
        ...
        st.markdown("---")
        st.markdown("**📎 Arquivos (opcional)**")
        st.caption("Tomografias, escaneamentos, fotos — max 200MB por arquivo.")
        uploads = st.file_uploader(
            "Arraste os arquivos aqui",
            accept_multiple_files=True,
            type=["dcm","stl","png","jpg","jpeg","pdf","zip","nrrd","obj"],
            key="pp_files_upload")
        if uploads:
            st.session_state["pp_files_pending"] = uploads
    ...
    # ── Passo 5: Revisão e Envio ──────────────────────────────
        ...
        n_arq = len(st.session_state.get("pp_files_pending", []))
        if n_arq:
            st.info(f"📎 {n_arq} arquivo(s) prontos para upload.")
        ...
        if col_env.button("🚀 Enviar Pedido para a 3D Guide",
                          type="primary", use_container_width=True):
            with st.spinner("Enviando pedido e fazendo upload dos arquivos…"):
                try:
                    # 1. Upload de arquivos → Supabase Storage
                    urls_arquivos = []
                    for f in st.session_state.get("pp_files_pending", []):
                        url = upload_arquivo(f.read(), f.name, f.type or "application/octet-stream")
                        urls_arquivos.append(url)
                    ...
                    payload = {
                        ...
                        "arquivos_paths":   "|".join(urls_arquivos),
                        "status":           "Caixa de Entrada",
                        "faturado":         False,
                        "empresa_destino":  "",
                    }
                    pedido_id = inserir_pedido(payload)
